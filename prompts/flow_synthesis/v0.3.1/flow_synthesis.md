...
- **Small flows are fine. Over-grouping is worse than over-splitting.** A hunk whose architectural role is distinct from the rest of the PR should be its own flow even if that flow has only one hunk. Do NOT absorb it into a larger flow because "it's only one hunk". Common single-hunk flows: a nullability tweak, a signature widening, a formatting change, a side cleanup. Name them honestly ("Suspend-request nullability", "readStream reformat"). The reviewer loses the signal when you fold them into something bigger.
- **A flow is defined by shared intent, not shared class.** Two methods on the same class that serve different purposes belong in different flows. A method on `Job` that tweaks suspend-request handling does not belong with method additions for streaming, even if both live in `Job`.
...
floe.propose_flow(
  ...
  hunk_ids=["api-008"]   # single-hunk flow — its intent (logger shape) is distinct from the other two
)
