...
use adr_core::{Flow, FlowSource, Graph, NodeKind};
...
pub fn neighbors(
    ...
    let mut visited: std::collections::HashSet<adr_core::NodeId> =
    ...
    let edges: Vec<NeighborEdge> = graph
        ...
        .filter_map(|e| {
            ...
            let kind = match &e.kind {
                adr_core::EdgeKind::Calls => NeighborEdgeKind::Calls,
                adr_core::EdgeKind::Defines => NeighborEdgeKind::Defines,
                adr_core::EdgeKind::Exports => NeighborEdgeKind::Exports,
                adr_core::EdgeKind::Transitions { from: f, to: t } => {
...
fn find_entity<'a>(graph: &'a Graph, qname: &str) -> Option<&'a adr_core::Node> {
...
fn node_to_descriptor(n: &adr_core::Node, side: SnapshotSide) -> EntityDescriptor {
...
fn hunk_to_summary(s: &Session, h: &adr_core::Hunk) -> HunkSummary {
    use adr_core::hunks::HunkKind;
