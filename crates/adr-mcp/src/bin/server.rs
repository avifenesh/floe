//! `adr-mcp` — MCP-over-stdio server binary.
//!
...
use adr_mcp::fs_tools::{self, GlobInput, GrepInput, ReadFileInput, ToolsRoot};
use adr_mcp::{ErrorCode, FinalizeOutcome, MutateFlowPatch, Session, ToolError};
...
const SERVER_NAME: &str = "adr-mcp";
const SERVER_VERSION: &str = env!("CARGO_PKG_VERSION");

#[derive(Parser, Debug)]
#[command(name = "adr-mcp", version, about = "MCP server for adr flow synthesis")]
struct Cli {
...
async fn dispatch(server: &Server, name: &str, args: Value) -> DispatchResult {
    ...
    let name = if let Some(rest) = name.strip_prefix("adr:") {
    ...
    match name.as_ref() {
        ...
        "adr.list_entities" => {
            ...
            struct A {
                #[serde(default)]
                side: Option<adr_mcp::wire::SnapshotSide>,
                #[serde(default)]
                kind: Option<adr_mcp::wire::EntityKindTag>,
            }
            let a: A = match serde_json::from_value(args) {
                Ok(v) => v,
                Err(e) => return DispatchResult::BadArgs(e.to_string()),
        ...
        "adr.read_file" => {
            let Some(root) = server.fs_root.as_ref() else {
                return DispatchResult::ProofOff("adr.read_file");
            };
            let a: ReadFileInput = match serde_json::from_value(args) {
                Ok(v) => v,
                Err(e) => return DispatchResult::BadArgs(e.to_string()),
            };
            match fs_tools::read_file(root, a) {
                Ok(v) => DispatchResult::Ok(serde_json::to_value(v).unwrap()),
                Err(e) => DispatchResult::FsErr(e.to_string()),
            }
        }
        "adr.grep" => {
            let Some(root) = server.fs_root.as_ref() else {
                return DispatchResult::ProofOff("adr.grep");
            };
            let a: GrepInput = match serde_json::from_value(args) {
                Ok(v) => v,
                Err(e) => return DispatchResult::BadArgs(e.to_string()),
            };
            match fs_tools::grep(root, a) {
                Ok(v) => DispatchResult::Ok(serde_json::to_value(v).unwrap()),
                Err(e) => DispatchResult::FsErr(e.to_string()),
            }
        }
        "adr.glob" => {
            let Some(root) = server.fs_root.as_ref() else {
                return DispatchResult::ProofOff("adr.glob");
            };
            let a: GlobInput = match serde_json::from_value(args) {
                Ok(v) => v,
                Err(e) => return DispatchResult::BadArgs(e.to_string()),
            };
            match fs_tools::glob(root, a) {
                Ok(v) => DispatchResult::Ok(serde_json::to_value(v).unwrap()),
                Err(e) => DispatchResult::FsErr(e.to_string()),
            }
        }
