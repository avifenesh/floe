...
//!
//! All cookies are signed with `ADR_SESSION_SECRET` (HMAC) so a
...
/// = active session (long-lived), `adr_os` = OAuth state (short-lived,
/// cleared immediately after callback exchange).
pub const SESSION_COOKIE: &str = "adr_s";
pub const OAUTH_STATE_COOKIE: &str = "adr_os";
...
pub struct AuthConfig {
    ...
    /// `ADR_FRONTEND_URL` for non-local dev (e.g.
    /// `http://127.0.0.1:5173`).
...
impl AuthConfig {
    /// Build from env. Returns `Ok(None)` when `ADR_SESSION_SECRET`
    /// is unset — auth is fully optional; missing secret disables
    /// every provider.
    pub fn from_env() -> anyhow::Result<Option<Self>> {
        let Some(secret) = std::env::var("ADR_SESSION_SECRET").ok().filter(|s| !s.is_empty()) else {
            tracing::info!("ADR_SESSION_SECRET unset — auth disabled");
            return Ok(None);
        };
        let key_bytes = hex::decode(&secret)
            .map_err(|e| anyhow!("ADR_SESSION_SECRET must be hex (openssl rand -hex 32): {e}"))?;
        if key_bytes.len() < 32 {
            return Err(anyhow!(
                "ADR_SESSION_SECRET too short ({} bytes) — need at least 32 bytes of entropy (64 hex chars)",
                key_bytes.len()
            ));
        }
        ...
        let mut reader = blake3::Hasher::new()
            .update(b"adr-session-key-v1|")
            .update(&key_bytes)
            .finalize_xof();
        reader.fill(&mut expanded);
        let session_key = Key::from(&expanded);

        let frontend_url = std::env::var("ADR_FRONTEND_URL")
...
/// cookie. **Only mounted when `ADR_ALLOW_DEV_LOGIN=1`.** Lets us test
