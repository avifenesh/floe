//! The agent loop — shuttles tool calls between Ollama and `adr-mcp`
...
async fn run(
    ...
    let hunks_json = mcp_call_json(&mut mcp, "adr.list_hunks", Value::Object(Default::default()))
        .await
        .context("pre-fetch list_hunks")?;
    let clusters_json =
        mcp_call_json(&mut mcp, "adr.list_flows_initial", Value::Object(Default::default()))
            .await
    ...
    let client = match cfg.provider {
        ...
        LlmProvider::Glm => {
            let key = cfg
                .api_key
                .clone()
                .ok_or_else(|| anyhow!("glm provider requires ADR_GLM_API_KEY"))?;
...
async fn dispatch_tool_call(
    ...
    let maybe_outcome = if name == "adr.finalize" && !result.is_error {
