//! Probe-side adapter. Wires `adr-probe`'s [`ProbeClient`] trait to the
//! existing Ollama / GLM chat clients plus an `adr-mcp` child process
//! for tool dispatch.
//!
//! Responsibilities:
//!
//! - **Chat**: forward to the wrapped [`Client`] (Ollama or GLM), map
//!   its [`ChatResponse`] onto [`adr_probe::ChatReply`].
...
use adr_probe::{ChatReply, Msg, ProbeClient, ToolDef, ToolDispatchResult};
...
impl McpProbeClient {
    ...
    pub fn new(
        ...
        let tools: Vec<ToolDef> = tool_specs
            ...
            .map(|spec| ToolDef {
                kind: "function",
                function: adr_probe::session::ToolDefFunction {
                    name: spec.name,
...
impl ProbeClient for McpProbeClient {
    async fn chat(
        ...
        // Convert `adr-probe`'s Msg/ToolDef into our ollama-shape types.
        let req = ChatRequest {
        ...
        Ok(ChatReply {
            message: Msg {
                ...
                tool_calls: resp
                    ...
                    .tool_calls
                    .into_iter()
                    .map(|tc| adr_probe::session::ToolCall {
                        function: adr_probe::session::ToolCallFunction {
...
pub async fn spawn_probe_mcp(artifact_path: &std::path::Path) -> Result<(Arc<Mutex<McpClient>>, Vec<super::mcp_client::ToolSpec>)> {
    ...
    .await
    .context("spawn adr-mcp for probe")?;
