...
//!   the process-wide `ADR_GLM_CONCURRENCY` semaphore + circuit
...
use adr_core::{Artifact, Flow, FlowSource, HunkKind};
...
/// the classic synth used (`ADR_LLM`). Returns a [`SynthesisOutcome`]
    ...
    let resp = match cfg.provider {
        ...
        LlmProvider::Glm => {
            let api_key = cfg
                .api_key
                .clone()
                .ok_or_else(|| anyhow!("glm without api key — check ADR_GLM_API_KEY"))?;
...
fn hunk_touches(_h: &adr_core::Hunk, _entity: &str) -> bool {
