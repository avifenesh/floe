...
//! sketch in `docs/floe-pi-extension.md` used), because:
//!
...
/// `floe:list_flows_initial()` result item.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// `floe:mutate_flow(id, patch)` request body.
...
/// `floe:finalize()` result. Either the accepted flow list or a reject
/// reason pointing at the invariant that failed.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "outcome", rename_all = "lowercase")]
pub enum FinalizeOutcome {
    Accepted {
        flows: Vec<floe_core::Flow>,
    },
    Rejected {
