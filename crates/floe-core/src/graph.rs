...
pub struct Node {
    ...
    /// Owning workspace package, when the root contained a pnpm/npm/
    /// yarn workspace or a `tsconfig.json` with `references`.
    /// `None` for single-package repos and for files outside any
    /// declared workspace glob. `floe-lsp` / `floe-hunks` use this to
    /// distinguish "intra-package" from "cross-package" call edges.
