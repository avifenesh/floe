//! Graph schema v0.1 — the machine-readable substrate every downstream crate reads.
//!
//! Schema versioning lives in [`Artifact::schema_version`]. Bump the minor on breaking
//! changes until v1. Every node and edge carries [`Provenance`] so downstream views can
//! explain where a fact came from.

pub mod graph;
pub mod provenance;
pub mod hunks;
pub mod cfg;
pub mod evidence;
pub mod flow;
pub mod flow_membership;
pub mod intent;
pub mod inline_notes;
pub mod artifact;

pub use artifact::{Artifact, ArtifactBaseline, CostStatus, PrSummary, ProofStatus, ReviewVerdict, ReviewVerdictRecord, Side, SynthStatus, SCHEMA_VERSION};
pub use cfg::{Cfg, CfgEdge, CfgEntry, CfgMap, CfgNode, CfgNodeId, CfgNodeKind};
pub use evidence::{
    Axes, Claim, ClaimKind, CompileDelta, CompileDiagnostic, Cost, CostDriver,
    CoverageDelta, CoverageFile,
    DiagnosticSeverity, ExternalRunDelta, ExternalRunOutcome, SourceRef, SourceSide, Strength,
};
pub use intent::{EvidenceType, Intent, IntentClaim, IntentInput};
pub use inline_notes::{FileLineSide, InlineNote, InlineNoteAnchor};
pub use flow::{Flow, FlowSource};
pub use flow_membership::{
    FlowMembership, MembershipDiagram, MembershipEdge, MembershipGroup, MembershipMember,
    MembershipShape,
};
pub use graph::{Edge, EdgeId, EdgeKind, Graph, Node, NodeId, NodeKind};
pub use hunks::{Hunk, HunkKind};
pub use provenance::Provenance;
