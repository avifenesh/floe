...
//! derived from the live `Artifact` type. Run `cargo run -p floe-cli -- schema >
...
fn schema_in_repo_matches_artifact_type() {
    ...
    let live = serde_json::to_string_pretty(&schemars::schema_for!(floe_core::Artifact))
    ...
    assert_eq!(
        norm(&live),
        norm(&committed),
        "schema.json is stale — run: cargo run -p floe-cli -- schema > schema.json"
    );
}
