...
fn qname(graph: &floe_core::Graph, id: floe_core::NodeId) -> Option<String> {
...
fn claim(
    ...
    Claim {
        ...
        provenance: Provenance::new(
            format!("floe-evidence/{collector}"),
            COLLECTOR_VERSION,
            format!("flow:{}", flow.id),
            flow.id.as_bytes(),
        ),
        // Structural-evidence claims come from tree-sitter-time,
        // before the LSP pass runs. The anchoring step fills in
        // source_refs on these afterwards — see
        // `floe_server::passes::claim_anchors`.
        source_refs: Vec::new(),
    }
}
...
mod tests {
    ...
    use floe_core::flow::FlowSource;
    use floe_core::graph::{Edge, EdgeId, EdgeKind, Node, NodeId, NodeKind, Span};
