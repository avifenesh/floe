...
pub fn compare(a: &Artifact, b: &Artifact) -> CalibrationReport {
    ...
    // Orphan = flow on this side that didn't end up in any pairing,
    // regardless of its best-Jaccard score. A "strong" unpaired flow
    // (outbid by another on the same side) is still something the
    // calibration report should surface — it tells the reader "side B
    // split the work more finely than side A did". Threshold was
    // blocking that signal and has been removed.
    let a_orphans = unpaired(&a.flows, &pairs, PairingSide::A, &b.flows);
    let b_orphans = unpaired(&b.flows, &pairs, PairingSide::B, &a.flows);
...
fn unpaired(
    ...
    let claimed: BTreeSet<&str> = pairs
        .iter()
        .map(|p| match side {
            PairingSide::A => p.a_name.as_str(),
            PairingSide::B => p.b_name.as_str(),
        })
        .collect();
    primary
        .iter()
        ...
        .map(|fp| {
            ...
            Orphan {
                ...
                best_match: if best_name.is_empty() {
                    None
                } else {
                    Some(best_name)
                },
...
fn greedy_pair(a: &[Flow], b: &[Flow]) -> Vec<FlowPairing> {
    ...
    for fa in a_sorted {
        ...
        if let Some((fb, j, shared, a_only, b_only)) = best {
            // Zero shared hunks isn't a pairing — it's two flows that
            // happen to be the only candidates. Leave them unpaired
            // so the calibration report lists them as orphans.
            if shared == 0 {
                continue;
            }
...
mod tests {
    use super::*;
    use floe_core::{artifact::PrRef, FlowSource};
