//! TypeScript parsing → `floe-core` graph.
//!
//! Scope 1 coverage:
...
use floe_core::graph::{Edge, EdgeId, EdgeKind, Graph, Node, NodeId, NodeKind, Span};
...
pub mod workspace;
pub use workspace::{tag_graph as tag_package_boundaries, PackageMap};
...
impl Ingest {
    ...
    fn ingest_file(&mut self, rel_path: &str, source: &[u8], all_files: &[String]) -> Result<()> {
        ...
        self.graph.nodes.push(Node {
            ...
            // Filled in post-ingest by workspace topology tagging.
            package: None,
        });
