...
use floe_core::{artifact::PrRef, Artifact, CostStatus, Flow, IntentInput, ProofStatus, SynthStatus};
...
pub struct PipelineRequest {
    ...
    /// When true, skip the cache-hit short-circuit so every pass
    /// (structural + synth + probe + proof + summary) fires afresh.
    /// Set by the rebaseline endpoint — "re-run now" should actually
    /// re-run, not serve the same cached artifact.
    pub force: bool,
}
...
async fn run_inner(req: PipelineRequest) -> Result<()> {
    ...
    // same synthesis model but different FLOE_PROOF_LLM (or one with
    ...
    let head_graph = {
        ...
        tokio::task::spawn_blocking(move || floe_parse::Ingest::new("head").ingest_dir(&head))
            .await
    ...
    let head_sha = {
        ...
        probe.snapshot_sha(floe_core::Side::Head)
    };
    ...
    // "ready" event. Bypassed when `force = true` (rebaseline), so
    // the reviewer gets a genuinely fresh run with all background
    // passes refiring — otherwise "Re-run now" silently served the
    ...
    let parse_base_task = {
        ...
        tokio::task::spawn_blocking(move || floe_parse::Ingest::new("base").ingest_dir(&base))
    };
    emit(job, "cfg", 35, "building control-flow graphs").await;
    let cfg_head_task = {
        let g = head_graph.clone();
        let root = head.to_path_buf();
        tokio::task::spawn_blocking(move || floe_cfg::build_for_graph(&g, &root))
    };
    ...
    // LSP enrichment — replace tree-sitter's syntactic call edges
    // with semantic ones from typescript-language-server. Toggle with
    // `FLOE_LSP_ENRICH` (default on; set to "0" to pin to the
    // tree-sitter floor for debugging). Best-effort: individual
    // failures fall back to the original edges on the affected side.
    let lsp_enabled = std::env::var("FLOE_LSP_ENRICH")
        .ok()
        .map(|v| v != "0" && !v.eq_ignore_ascii_case("false"))
        .unwrap_or(true);
    let (mut base_graph, mut head_graph) = if lsp_enabled {
        emit(job, "lsp-enrich", 70, "resolving cross-file calls via LSP").await;
        let base_root = base.to_path_buf();
        let head_root = head.to_path_buf();
        let base_fut = floe_lsp::enrich_graph(base_graph, &base_root);
        let head_fut = floe_lsp::enrich_graph(head_graph, &head_root);
        tokio::join!(base_fut, head_fut)
    } else {
        (base_graph, head_graph)
    };

    // Workspace topology — scan monorepo manifests once per side and
    // tag every node with its owning package. Single-repo checkouts
    // get an empty map; `tag_graph` is a no-op in that case.
    {
        let base_map = floe_parse::PackageMap::load(base);
        floe_parse::tag_package_boundaries(&mut base_graph, &base_map);
        let head_map = floe_parse::PackageMap::load(head);
        floe_parse::tag_package_boundaries(&mut head_graph, &head_map);
    }

    emit(job, "hunks", 75, "extracting semantic hunks").await;
    let mut hunks = floe_hunks::extract_all(&base_graph, &head_graph);
    hunks.extend(floe_hunks::extract_lock_hunks(base, head));
    hunks.extend(floe_hunks::extract_data_hunks(base, head));
    hunks.extend(floe_hunks::extract_docs_hunks(head));
    hunks.extend(floe_hunks::extract_deletion_hunks(&base_graph, &head_graph));
    ...
    artifact = floe_evidence::collect(artifact);

    // Claim anchoring — every claim with entities gets a best-effort
    // `SourceRef` lifted from the graph's byte span + file lookup.
    // Compile / external-runs passes already emit their own refs; this
    // step fills the gap on structural-evidence claims so the UI's
    // "jump to source" affordance lights up everywhere. Fast and
    // synchronous — no LSP call; it reads cached file content.
    crate::passes::claim_anchors::attach(&mut artifact);
    ...
    // Intent extraction pass — synthesise a structured intent from
    // commit messages / README / changed filenames when the caller
    // didn't supply one. Runs BEFORE we gate the proof pass so a
    // successful extraction still enables proof. Default model is
    // local Qwen; override via `FLOE_INTENT_LLM`.
    if artifact.intent.is_none() {
        if let Some(intent_cfg) = LlmConfig::from_env_intent() {
            if let Some(extracted) =
                crate::passes::intent_extract::run(&intent_cfg, base, head).await
            {
                tracing::info!("intent extraction pass filled artifact.intent from repo signals");
                artifact.intent = Some(extracted);
            } else {
                artifact
                    .notices
                    .push("Intent extraction skipped — local model unreachable. Proof coverage will be limited.".into());
            }
        }
    }
    ...
    // `from_env_proof` defaults to GLM-4.7 when `FLOE_GLM_API_KEY`
    ...
    *job.status.write().await = JobStatus::Ready;
    ...
    // PR-summary pass — runs in parallel with synth/probe/proof. Cheap
    // one-shot GLM call (GLM-4.6 by default) that writes a
    // reviewer-facing headline + optional description to
    // `artifact.pr_summary`. UI swaps `repo#N` for the headline as soon
    // as this lands.
    if let Some(summary_cfg) = LlmConfig::from_env_summary() {
        let job_bg = Arc::clone(job);
        let cache_bg = Arc::clone(cache);
        let key_bg = key.clone();
        let cache_eligible = true;
        let artifact_for_summary = artifact.clone();
        tokio::spawn(async move {
            run_summary_pass(
                job_bg,
                cache_bg,
    ...
    // Compile-unit pass — runs `tsc --noEmit` on base + head and
    // diffs the diagnostics. Fully parallel to probe/proof/summary;
    // the result writes back via `merge_into_artifact`. Skipped
    // silently when tsconfig is missing or `FLOE_COMPILE_PASS=0`.
    {
        let job_bg = Arc::clone(job);
        let cache_bg = Arc::clone(cache);
        let key_bg = key.clone();
        ...
        tokio::spawn(async move {
            run_compile_pass(job_bg, cache_bg, cache_eligible, key_bg, base_path, head_path).await;
        });
    }

    // External-run passes — `FLOE_TEST_CMD` / `FLOE_BENCH_CMD` when
    // set. Tests emit pass↔fail transition claims; bench output is
    // captured verbatim for the proof pass to mine. Both are
    // background passes and merge back via `merge_into_artifact`.
    for kind in [
        crate::passes::external_runs::RunKind::Tests,
        crate::passes::external_runs::RunKind::Bench,
    ] {
        let job_bg = Arc::clone(job);
        let cache_bg = Arc::clone(cache);
        let key_bg = key.clone();
    ...
    // Coverage-delta pass — parses lcov / json-summary on both sides
    // and attaches `CoverageDrop` claims. No-op when no coverage file
    // exists, so safe to always run.
    {
        let job_bg = Arc::clone(job);
        let cache_bg = Arc::clone(cache);
        let key_bg = key.clone();
        ...
        tokio::spawn(async move {
            let mut snapshot = {
                let guard = job_bg.artifact.read().await;
                match guard.as_ref() {
                    Some(a) => a.clone(),
                    None => return,
                }
            };
            crate::passes::coverage::attach(&mut snapshot, &base_path, &head_path).await;
            merge_into_artifact(&job_bg, &cache_bg, true, &key_bg, |current| {
                current.coverage_delta = snapshot.coverage_delta.clone();
                for flow in &snapshot.flows {
                    if let Some(target) = current.flows.iter_mut().find(|f| f.id == flow.id) {
                        for claim in &flow.evidence {
                            if matches!(claim.kind, floe_core::ClaimKind::CoverageDrop)
                                && !target.evidence.iter().any(|c| {
                                    matches!(c.kind, floe_core::ClaimKind::CoverageDrop)
                                        && c.text == claim.text
                                })
                            {
                                target.evidence.push(claim.clone());
                            }
                        }
                    }
                }
    ...
    // Claim-anchor fan-out — extend existing source_refs with LSP
    // reference sites so every claim carries both declaration and
    // usage anchors. Background; `attach` already ran synchronously.
    {
        let job_bg = Arc::clone(job);
        let cache_bg = Arc::clone(cache);
        let key_bg = key.clone();
        ...
        let head_path = head.to_path_buf();
        tokio::spawn(async move {
            run_claim_anchor_fanout(job_bg, cache_bg, key_bg, base_path, head_path).await;
        });
    }
...
async fn run_external_pass(
    ...
    cache_eligible: bool,
    key: String,
    kind: crate::passes::external_runs::RunKind,
    base: std::path::PathBuf,
    head: std::path::PathBuf,
) {
    let stage = match kind {
        crate::passes::external_runs::RunKind::Tests => "tests",
        crate::passes::external_runs::RunKind::Bench => "bench",
    };
    let _ = job.progress.send(ProgressEvent {
        stage: format!("external-{stage}"),
        percent: 0,
        message: format!("Running {stage} on base + head"),
    });
    let delta = crate::passes::external_runs::run(kind, &base, &head).await;
    match delta {
        Some(d) => {
            merge_into_artifact(&job, &cache, cache_eligible, &key, |current| {
                match kind {
                    crate::passes::external_runs::RunKind::Tests => {
                        current.test_run = Some(d);
                    }
                    crate::passes::external_runs::RunKind::Bench => {
                        current.bench_run = Some(d);
                    }
                }
                crate::passes::external_runs::attach_claims(current);
            })
            .await;
        }
        None => {
            tracing::info!(kind = stage, "external run pass: unconfigured, skipping");
        }
    }
}

async fn run_compile_pass(
    job: Arc<Job>,
    cache: Arc<Cache>,
    cache_eligible: bool,
    key: String,
    ...
    let _ = job.progress.send(ProgressEvent {
        stage: "compile-unit".into(),
        percent: 0,
        message: "Type-checking base + head".into(),
    });
    let delta = crate::passes::compile::run(&base, &head).await;
    match delta {
        Some(d) => {
            tracing::info!(
                new = d.new_on_head.len(),
                resolved = d.resolved_on_head.len(),
                persistent = d.persistent.len(),
                both_ran = d.both_ran,
                "compile pass finished"
            );
            merge_into_artifact(&job, &cache, cache_eligible, &key, |current| {
                current.compile_diagnostics = Some(d);
                crate::passes::compile::attach_claims(current);
            })
            .await;
        }
        None => {
            tracing::info!("compile pass: skipped (disabled or no tsconfig)");
        }
    }
}
...
async fn run_summary_pass(
    ...
    let _ = job.progress.send(ProgressEvent {
        stage: "pr-summary".into(),
        percent: 0,
        message: "Drafting PR headline".into(),
    });
    match crate::llm::summary_pass::run_summary(&cfg, &snapshot).await {
        Ok(summary) => {
            tracing::info!(headline = %summary.headline, "pr summary pass ok");
            merge_into_artifact(&job, &cache, cache_eligible, &key, |current| {
                current.pr_summary = Some(summary);
            })
            .await;
        }
        Err(e) => {
            tracing::warn!(error = %e, "pr summary pass failed — leaving pr_summary unset");
        }
    }
}
    ...
    match outcome {
        SynthesisOutcome::Accepted(flows) => {
            ...
            merge_into_artifact(&job, &cache, true, &key, |current| {
                ...
                // Baseline.synthesis_model is plucked by floe-cost at
                ...
                if let Some(baseline) = current.baseline.as_mut() {
                    let synth_model = current.flows.iter().find_map(|f| match &f.source {
                        floe_core::FlowSource::Llm { model, .. } => Some(model.clone()),
                        floe_core::FlowSource::Structural => None,
                    });
        ...
        SynthesisOutcome::Rejected { rule, detail } => {
            ...
            merge_into_artifact(&job, &cache, true, &key, |current| {
                ...
                // Stamp the attempted model even on rejection so the
                // drift banner doesn't loop forever saying "synthesis
                // was skipped" — the reviewer's run did try, just with
                // an unacceptable LLM output. `synth_status=Errored`
                // carries the genuine failure signal elsewhere.
                stamp_synth_model_if_absent(current, &cfg.model);
            })
            .await;
            let _ = job.progress.send(ProgressEvent {
                stage: "llm-synthesize".into(),
        ...
        SynthesisOutcome::NoFinalize | SynthesisOutcome::Errored(_) => {
            ...
            merge_into_artifact(&job, &cache, true, &key, |current| {
                current.synth_status = SynthStatus::Errored;
                // See comment in the Rejected arm — stamp the model so
                // subsequent visits don't loop the drift banner forever.
                stamp_synth_model_if_absent(current, &cfg.model);
            })
            .await;
            let _ = job.progress.send(ProgressEvent {
...
async fn run_probe_pass(inputs: ProbePassInputs) {
    ...
    let baseline_root = cache.root().parent().map(|p| p.join("baseline")).unwrap_or_else(|| PathBuf::from(".floe/baseline"));
    ...
    match pipeline.run(&artifact).await {
        Ok(outcome) => {
            ...
            artifact = floe_cost::attribute_from_baselines(artifact, &outcome.base, &outcome.head);
...
async fn run_intent_pass(
    ...
    merge_into_artifact(&job, &cache, cache_eligible, &key, |current| {
        ...
        // Stamp the proof model we attempted — even on error — so
        // the drift banner doesn't keep saying "proof was skipped"
        // across sessions. `proof_status=Errored` surfaces the
        // genuine failure signal.
        stamp_proof_model_if_absent(current, &proof_cfg.model);
...
async fn run_claim_anchor_fanout(
    job: Arc<Job>,
    cache: Arc<Cache>,
    key: String,
    ...
    // Snapshot current artifact, run the (slow) LSP fan-out on the
    // clone, then merge source_refs back by (flow_id, claim index).
    let mut snapshot = {
        let guard = job.artifact.read().await;
        match guard.as_ref() {
            Some(a) => a.clone(),
            None => return,
        }
    };
    crate::passes::claim_anchors::fan_out_refs(&mut snapshot, &base_path, &head_path).await;
    merge_into_artifact(&job, &cache, true, &key, |current| {
        for flow in &snapshot.flows {
            let Some(target) = current.flows.iter_mut().find(|f| f.id == flow.id) else {
                continue;
            };
            for (ci, claim) in flow.evidence.iter().enumerate() {
                let Some(target_claim) = target.evidence.get_mut(ci) else {
                    continue;
                };
                for r in &claim.source_refs {
                    let dupe = target_claim
                        .source_refs
                        .iter()
                        .any(|e| e.side == r.side && e.file == r.file && e.line == r.line);
                    if !dupe {
                        target_claim.source_refs.push(r.clone());
                    }
                }
            }
        }
...
/// same cache entry.
/// Stamp `baseline.synthesis_model` to `model` when absent.
///
/// Called from every synth-outcome branch — success, rejection, and
/// error — so the drift banner's "was <skipped>" message clears once
/// we've attempted synthesis with a configured model, regardless of
/// whether the LLM produced an acceptable output. `synth_status`
/// still carries the real success/failure signal; this just stops the
/// banner from nagging on every subsequent visit.
///
/// Creates a minimal baseline when one doesn't exist yet (synth can
/// land before probe on fast/small repos).
fn stamp_synth_model_if_absent(current: &mut Artifact, model: &str) {
    let stamped = model.to_string();
    match current.baseline.as_mut() {
        Some(b) => {
            if b.synthesis_model.is_none() {
                b.synthesis_model = Some(stamped);
            }
        }
        None => {
            current.baseline = Some(floe_core::ArtifactBaseline {
                axes_base: floe_core::Axes::default(),
                tokens_base: 0,
                tokens_head: 0,
                probe_model: String::new(),
                probe_set_version: String::new(),
                synthesis_model: Some(stamped),
                proof_model: None,
            });
        }
    }
...
/// Symmetric to `stamp_synth_model_if_absent` for the proof pass.
fn stamp_proof_model_if_absent(current: &mut Artifact, model: &str) {
    let stamped = model.to_string();
    match current.baseline.as_mut() {
        Some(b) => {
            if b.proof_model.is_none() {
                b.proof_model = Some(stamped);
            }
        }
        None => {
            current.baseline = Some(floe_core::ArtifactBaseline {
...
fn write_artifact_tmp(artifact: &Artifact) -> Result<PathBuf> {
    ...
    let fname = format!("floe-llm-{}.json", uuid::Uuid::new_v4());
