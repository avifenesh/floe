...
//! notes) and the same MCP toolbox (`floe.read_file` / `floe.grep` /
//! `floe.glob` + `floe.get_entity` / `floe.neighbors`). They run
    ...
    let content = match cfg.provider {
        LlmProvider::Glm => {
            let key = cfg
                .api_key
                .clone()
                .ok_or_else(|| anyhow!("FLOE_GLM_API_KEY required for intent summarization"))?;
...
async fn run_session(
...
) -> Result<Value> {
    {
        tracing::info!(
            ...
            "spawning floe-mcp (proof mode) for session"
        );
        ...
        .await
        .context("spawning floe-mcp in proof mode")?;
    ...
    if !i.claims.is_empty() {
        ...
        for (idx, c) in i.claims.iter().enumerate() {
            let kind = match c.evidence_type {
                floe_core::EvidenceType::Bench => "bench",
                floe_core::EvidenceType::Example => "example",
                floe_core::EvidenceType::Test => "test",
                floe_core::EvidenceType::Observation => "observation",
            };
...
fn parse_intent_fit(v: Value, model: &str, version: &str) -> Result<IntentFit> {
    use floe_core::{intent::IntentFitVerdict, Strength};
...
fn parse_proof(v: Value, model: &str, version: &str) -> Result<Proof> {
    use floe_core::intent::{ClaimProofKind, ClaimProofStatus, EvidenceType, ProofEvidence, ProofVerdict};
    let verdict = str_field(&v, "verdict")?;
    let verdict = match verdict.as_str() {
...
fn parse_strength(v: &Value) -> Result<floe_core::Strength> {
    use floe_core::Strength;
...
fn write_tmp_artifact(artifact: &Artifact) -> Result<PathBuf> {
    ...
    let fname = format!("floe-intent-{}.json", uuid::Uuid::new_v4());
...
mod tests {
    ...
    fn parse_inline_glm_tool_calls_single() {
        let content = "Let me check: <tool_call>floe.read_file<arg_key>file_path</arg_key><arg_value>src/foo.ts</arg_value></tool_call>";
        let got = parse_inline_glm_tool_calls(content);
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].0, "floe.read_file");
    ...
    fn parse_inline_glm_tool_calls_multiple_with_int_and_bool_coercion() {
        let content = "<tool_call>floe.grep<arg_key>pattern</arg_key><arg_value>foo</arg_value><arg_key>limit</arg_key><arg_value>30</arg_value><arg_key>case_insensitive</arg_key><arg_value>true</arg_value></tool_call><tool_call>floe.glob<arg_key>pattern</arg_key><arg_value>**/*.ts</arg_value></tool_call>";
        let got = parse_inline_glm_tool_calls(content);
        assert_eq!(got.len(), 2);
        assert_eq!(got[0].0, "floe.grep");
        assert_eq!(got[0].1.get("limit").unwrap().as_i64(), Some(30));
        assert_eq!(got[0].1.get("case_insensitive").unwrap().as_bool(), Some(true));
        assert_eq!(got[1].0, "floe.glob");
    }
