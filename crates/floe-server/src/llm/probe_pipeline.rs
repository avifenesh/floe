...
//! milestone, rewriting `floe-cost`). This module only handles probe
//! execution + baseline storage.

use std::path::{Path, PathBuf};
use std::sync::Arc;

use floe_core::{Artifact, Side};
use floe_probe::{
    ...
    match cfg.provider {
        ...
        LlmProvider::Glm => {
            let key = cfg
                .api_key
                .clone()
                .context("glm provider requires FLOE_GLM_API_KEY for the probe pass")?;
...
fn write_tmp_artifact(a: &Artifact) -> Result<PathBuf> {
    ...
    let fname = format!("floe-probe-{}.json", uuid::Uuid::new_v4());
