//! Minimal MCP client that spawns `floe-mcp` as a child over stdio and
...
impl McpClient {
    /// Spawn `floe-mcp` with the given artifact. The binary path defaults
    /// to `floe-mcp` (looked up in PATH); override via `FLOE_MCP_BIN`.
    pub async fn spawn(
        artifact: &Path,
        model: &str,
        runtime_version: &str,
    ...
    /// Spawn `floe-mcp` in probe mode (relaxes the flows-required
    ...
    async fn spawn_inner(
        ...
        let bin = std::env::var("FLOE_MCP_BIN").unwrap_or_else(|_| "floe-mcp".into());
        ...
        let mut child = cmd
            .spawn()
            .with_context(|| format!("spawning floe-mcp binary `{bin}`"))?;
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(l)) = lines.next_line().await {
                tracing::debug!(target: "floe-mcp", "{l}");
            }
        });
        ...
        let resp = self
            .request(
                ...
                json!({
                    ...
                    "capabilities": {},
                    "clientInfo": { "name": "floe-server", "version": env!("CARGO_PKG_VERSION") },
                }),
            )
            .await?;
