...
/// without tripping the limiter. Override via `FLOE_GLM_CONCURRENCY`.
...
fn glm_semaphore() -> &'static Arc<Semaphore> {
    ...
    SEM.get_or_init(|| {
        let permits = std::env::var("FLOE_GLM_CONCURRENCY")
            .ok()
        ...
        tracing::info!(
            permits,
            "glm concurrency semaphore armed — configure FLOE_GLM_CONCURRENCY to change"
        );
...
impl GlmClient {
    ...
    /// 1. **Process-wide semaphore** (`FLOE_GLM_CONCURRENCY`) — only N
    ...
    pub async fn chat(&self, req: ChatRequest) -> Result<ChatResponse> {
        ...
        if std::env::var("FLOE_LLM_DEBUG").is_ok() {
...
fn parse_glm_response(body: &str) -> Result<ChatResponse> {
    ...
    if content_len == 0 && tool_calls_count == 0 {
        ...
        // If the model put its answer in reasoning_content, promote
        // it to content so the downstream JSON extractor / agent loop
        // sees a non-empty turn. Previously we only did this on a
        // clean finish_reason, which left `length`-truncated
        // reasoning responses empty — GLM-4.6 routinely burns the
        // whole budget on reasoning and never writes to `content`,
        // so excluding truncated-reasoning made every summary-pass
        // call effectively blank. Even truncated reasoning often
        // contains a complete JSON object inside a ```json fence;
        // `extract_json_object` tolerates the prose around it.
        if reasoning_len > 0 {
