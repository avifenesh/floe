...
> `floe-mcp` — a Rust binary speaking MCP-over-stdio JSON-RPC 2.0.
> `floe-server` spawns it as a child per analysis; the tool surface,
> error codes, and host invariants below survived the pivot intact.
>
> **Current source of truth** for the tool contract is the code —
> `crates/floe-mcp/src/handlers.rs`, `wire.rs`, `errors.rs`, plus the
...
- **read tools** that expose the analyzed artifact (the graph, hunks, starting clusters) — stable `floe:list_hunks`, `floe:get_entity`, `floe:neighbors`, `floe:list_flows_initial`
- **mutation tools** that let the model record flow assignments, validated host-side — `floe:propose_flow`, `floe:mutate_flow`, `floe:remove_flow`, `floe:finalize`

The host (Rust, in `floe-mcp`) exposes the artifact over a per-run local socket (Unix domain socket on POSIX, named pipe on Windows) and validates every mutation before accepting it. Whole-run rejection on any invariant violation.
...
1. `floe-server` writes the artifact to a temp path.
2. `floe-mcp` binds a per-run socket, e.g. `\\.\pipe\adr-<jobId>` on Windows, `/tmp/adr-<jobId>.sock` on POSIX.
...
6. On `floe:finalize`, the extension posts final flow list to the host socket and waits for accept/reject.
...
Lives in `crates/floe-mcp/prompts/flow_synthesis.md` and is versioned. Single source of truth, templated at runtime with the hunk count and structural cluster count.

Key instructions to the model:

- You are classifying hunks into flows — not writing code.
- Use `floe:list_hunks` and `floe:list_flows_initial` to see the starting point.
- Call `floe:get_entity` / `floe:neighbors` / PI's `read` / PI's `grep` to inspect what you don't understand.
...
- Every entity you reference must exist — use the IDs from `floe:get_entity`. Never invent.
...
### `floe:neighbors(id: string, hops?: number = 1)`
...
- `hunk_ids`: string[], each must appear in `floe:list_hunks()`.
- `extra_entities`?: string[], each must appear via `floe:get_entity()`. These are non-hunk entities the LLM is adding to express the flow's reach (unchanged callers/callees that belong to the flow).
...
Mode is picked by the host based on model name + artifact size. Override via `.floe/llm.toml::mode`.
...
- Host-side host: `floe-mcp` crate — versioned alongside the rest of the workspace.
- Compatibility matrix maintained in `crates/floe-mcp/COMPAT.md` when we actually break the wire.
